...
_Last updated: 2026-05-02 — **Upstream transit org name in IP table**: the Network Intelligence IP table now has an **Upstream** column (between ASN and Bytes) showing the transit provider's company name (e.g. "AT&T Enterprises, LLC") resolved via a new `_cymru_asn_names_bulk` function in `web_exposure.py`. Names are fetched once via Team Cymru bulk whois (verbose/begin-end format, ASN→org), cleaned of country suffix, and cached in `ip_intel.json` under `__upstream_asns__` so subsequent report runs need no network call. The ASN pill tooltip no longer duplicates the upstream (removed "Via:" line). Prior: **Offline BGP prefix lookup**: `vpn-leaks bgp-update` downloads the latest Route Views MRT RIB (~73 MB compressed) and builds `.cache/vpn_leaks/bgp_prefixes.db` (SQLite, 1.09M IPv4 prefixes, integer-range LPM index). `bgp_lookup.lookup_ip(ip)` returns `{asn, upstream_asn, prefix, as_path}` in sub-milliseconds with no network dependency; integrated into `pcap_host_intelligence` (fills ASN when ARIN whois fails; adds prefix + upstream ASN to each IP row) and `attribution/merge.py`. Team Cymru kept as last-resort fallback. Dig results cached in `ip_intel.json` — second report run ~3s. `geolite_asn.lookup_city()` for GeoLite2-City MMDB. Prior: capture-first HTML report redesign, persistent IP intel cache, Cymru bulk ASN lookup, heuristic + LLM org-name normalization._
