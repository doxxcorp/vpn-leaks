...
_Last updated: 2026-05-02 — **Capture-first HTML report redesign**: `VPNs/<SLUG>.html` now leads with a **Network Intelligence workspace** (PCAP-derived company → ASN → IP clusters, sticky nav with leak chips, KPI row, cluster search) above the fold; SPEC framework, web surface, exposure graph, and full narrative are collapsed behind `<details>` (Tier 3). Added **persistent IP intelligence cache** (`.cache/vpn_leaks/ip_intel.json`, 30-day TTL) so `pcap_host_intelligence` skips live whois/rDNS on repeat runs. **Bulk Team Cymru ASN lookup** (`_cymru_asn_bulk`, single TCP connection to `whois.cymru.com:43`) fills ASNs that standard ARIN/RIPE whois omits. **Heuristic org-name normalization** groups WHOIS handle variants (GOOGL-2/GOGL/GOOGLE → Google, AMAZO-4/AMAZON-EC2-USEAST-10 → Amazon, etc.) via rDNS suffix + owner prefix tables; **Claude Opus LLM** normalizer enhances when `ANTHROPIC_API_KEY` is set. IP table in each cluster gains an **ASN pill** with CSS hover tooltip showing WHOIS details. Prior: per-location PCAP host intelligence, report-generation progress phases, `run --with-pcap`, `graph_schema` 1.1 PCAP edges._
...
| Reporting | [vpn_leaks/reporting/generate_reports.py](vpn_leaks/reporting/generate_reports.py), [html_dashboard.py](vpn_leaks/reporting/html_dashboard.py), [web_exposure.py](vpn_leaks/reporting/web_exposure.py) (`methodology_and_pcap_sections`, `pcap_host_intelligence`, `build_capture_workspace_rollup`, `_cymru_asn_bulk`, `_heuristic_canonical`, `_normalize_org_names_llm`, **IP intel cache**), [exposure_graph.py](vpn_leaks/reporting/exposure_graph.py) (**`graph_schema` 1.1**), Jinja templates, [static/](vpn_leaks/reporting/static/) (CSS + isotype) | `VPNs/<SLUG>.md` + **`VPNs/<SLUG>.html`** (capture-first layout: PCAP workspace Tier 1 → benchmark run cards Tier 2 → SPEC/web/graph/narrative Tier 3 behind `<details>`), `PROVIDERS/AS<n>.md`, **`graph-export`** JSON |
