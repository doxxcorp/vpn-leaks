...
_Last updated: 2026-05-02 — **Offline BGP prefix lookup**: `vpn-leaks bgp-update` downloads the latest Route Views MRT RIB (~73 MB compressed) and builds `.cache/vpn_leaks/bgp_prefixes.db` (SQLite, 1.09M IPv4 prefixes, integer-range LPM index). `bgp_lookup.lookup_ip(ip)` returns `{asn, upstream_asn, prefix, as_path}` in sub-milliseconds with no network dependency; integrated into `pcap_host_intelligence` (fills ASN when ARIN whois fails; adds prefix + upstream ASN to each IP row and HTML tooltip) and `attribution/merge.py` (added as first source in tunnel exit attribution). Team Cymru kept as last-resort fallback. Dig results (PTR for IPs; A/AAAA/CNAME/MX/TXT for SNI/DNS hostnames) added to `ip_intel.json` cache — second report run now completes in ~3s. `geolite_asn.lookup_city()` added for GeoLite2-City MMDB (country/region/city; configure via `geolite_city_path` in `configs/tools/attribution.yaml`). Prior: capture-first HTML report redesign, persistent IP intel cache, Cymru bulk ASN lookup, heuristic + LLM org-name normalization._
